...
namespace organizedposts {
    export namespace app {
        /** 列表应用-职位 */
        export class PositionListApp extends ibas.BOListApplication<IPositionListView, bo.Position> {
            /** 应用标识 */
            static APPLICATION_ID: string = "855d4a3d-5aaf-4c77-bdcf-d98466526247";
            /** 应用名称 */
            static APPLICATION_NAME: string = "organizedposts_app_position_list";
            /** 业务对象编码 */
            static BUSINESS_OBJECT_CODE: string = bo.Position.BUSINESS_OBJECT_CODE;
            /** 构造函数 */
            constructor() {
                super();
                this.id = PositionListApp.APPLICATION_ID;
                this.name = PositionListApp.APPLICATION_NAME;
                this.boCode = PositionListApp.BUSINESS_OBJECT_CODE;
            ...
            /** 查看数据，参数：目标数据 */
            protected viewData(data: bo.Position): void {
                // 检查目标数据
            ...
            protected deleteData(data: bo.Position | bo.Position[]): void {
                let beDeleteds: ibas.IList<bo.Position> = ibas.arrays.create(data);
                // 没有选择删除的对象
